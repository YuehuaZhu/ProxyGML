...
def eval_metrics_one_dataset(model, test_dataloader, device, k_vals, opt):
    ...
    model.eval()
    n_classes = len(test_dataloader.dataset.class_to_idx)#len(test_dataloader.dataset.avail_classes)
    ...
    with torch.no_grad():
        ...
        for idx,inp in enumerate(final_iter):
            ...
            #input_img = input_img.cuda(opt.gpu, non_blocking=True)
...
def set_checkpoint(model, opt, progress_saver, savepath):
    ...
    Returns:
        Nothing!
    """
    if opt.new_save_checkpoint:
        torch.save({'state_dict':model.state_dict(), 'opt':opt,'progress':progress_saver}, savepath)
...
def metrics_to_examine(dataset, k_vals):
    ...
    if dataset=='vehicle_id':
        ...
        metric_dict['val'] = ['Epochs', 'Time', 'NMI']
